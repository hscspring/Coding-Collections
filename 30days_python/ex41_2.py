...
def central_corridor():
	'''You should choose "dodge!", "shoot!", or "tell a joke".

	The Gothons of Planet Percal #25 have invaded your ship and destroyed your entire crew. 
	You are the last surviving member and your last mission is to get the neutron destrunct bomb from the Weapons Armory, put it in the bridge, and blow the ship up after getting into an escape pod.
...
def laser_weapon_armory():
	'''You should input a 3 digits' number.
	...
	code = "%d%d%d" % (9, 2, 7) # this means code is what you input
...
def the_bridge():
	'''You should choose "throw the bomb" or "slowly place the bomb".
...
def escape_pod():
	'''You should input a single number.
	...
	Some of them could be demaged but you don't have time to look. 
...
def runner(map, start):
	...
	next = start
	while True:
		next = map[next]() # because each def return a string, not a function, for example return 'laser_weapon_armory'
#	start = map[start]()

		room = map[next] # next = 'central_corridor', then room is fun: central_corridor
		print "\n----------"
		next = room() # it's a loop to go for next. for example, first time it's the function you have played just now.
